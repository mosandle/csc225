...
public final class VirtualWorld //reviewed and confirmed done
   extends PApplet
{
   ...
   */
   ...
   public void mouseClicked()   {
      ...
      String BARREL_KEY = "barrel";
      Barrel barrel = Barrel.createBarrel("barrel", pressed, imageStore.getImageList(BARREL_KEY));

      Optional<Entity> maybeOccupied = world.getOccupant(pressed);
      if (maybeOccupied.isPresent()){
         if(maybeOccupied.get() instanceof Barrel){
         world.removeEntity(maybeOccupied.get());
         //scheduler.unscheduleAllEvents(maybeOccupied.get());
         }
      }
      else{
         world.addEntity(barrel);
      barrel.executeActivity( world,  imageStore,  scheduler);}
      //Schedule barrel actions
   ...
   {
      System.out.println("working directory equals " + System.getProperty("user.dir"));
